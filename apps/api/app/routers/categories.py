...
from app.schemas.category import CreateCategoryInput, UpdateCategoryInput, CategoryResponse
...
async def create_category(data: CreateCategoryInput, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    category = Category(
        user_id=user.id,
        name=data.name,
        type=data.type,
...
async def update_category(category_id: str, data: UpdateCategoryInput, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
