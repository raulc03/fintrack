...
class Obligation(Base):
    ...
    carryover_amount: Mapped[float] = mapped_column(
        Numeric(12, 2), nullable=False, default=0
    )
    currency: Mapped[str] = mapped_column(String(3), nullable=False)
    due_day: Mapped[int] = mapped_column(Integer, nullable=False)
    linked_movement_id: Mapped[str | None] = mapped_column(
        String, ForeignKey("movements.id"), nullable=True
    )
...
class ObligationHistory(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "obligation_id", "month_start", name="uq_obligation_history_month"
        ),
    )
    ...
    obligation_id: Mapped[str] = mapped_column(
        String, ForeignKey("obligations.id"), nullable=False, index=True
    )
