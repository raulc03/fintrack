...
async def seed_dev_data() -> None:
    async with async_session() as session:
        ...
        checking_usd = _add_account(user, "BCP Checking", "USD", "#3b82f6", 450)
        savings_pen = _add_account(
            user, "Interbank Soles Ahorro", "PEN", "#10b981", 2077
        )
        savings_usd = _add_account(
            user, "Interbank Dolares Ahorro", "USD", "#8b5cf6", 250
        )
        broker_usd = _add_account(user, "Broker USD", "USD", "#f59e0b", 900)
        ...
        movements = [
            ...
            _add_movement(
                ...
                date=_dt(20),
                category_id=categories["Transfer"].id,
                destination_account=broker_usd,
            ),
        ]
